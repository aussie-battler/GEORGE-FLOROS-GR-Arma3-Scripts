...
diag_log "//________________ Trader_rvg_arms_s.sqf _____________";
...
_Building = selectRandom GF_Trader_arms_Building_array;
...
_Trader_arms_group = createGroup civilian;
_Trader_arms = _Trader_arms_group createUnit ["C_Orestes", _Trader_Pos, [], 0, "FORM"];

//________________ Dress the trader with Ravage selections	________________
0 = [_Trader_arms] call rvg_fnc_equip; 
_Trader_arms setDir (random 360);

_Trader_arms setVariable ["isTrader", "rvg_arms_s", true];
_Trader_arms setVariable ["GF_rvg_arms_s", true];
...
[[_Trader_arms,_ambient_Anim_array, "NONE"],BIS_fnc_ambientAnim ] remoteExec ["call"];
_Trader_arms allowDamage false;
_Trader_arms disableAI "MOVE";
...
{
		if (
		((_x getVariable ["GF_rvg_arms_s",false]))		
		) then {
		_marker = createMarker ["GF_Trader_arms" + str(_forEachIndex), getPos _x];
		_marker setMarkerShape "ICON";
		_marker setMarkerType "mil_marker";	
		_marker setMarkerColor "ColorRed";
		_marker setMarkerSize [1,1];
		_marker setMarkerText "Arms Trader"; 
	}; 
} 
forEach allMissionObjects "All";
};

systemchat "A r m s  T r a d e r s   S p a w n e d";