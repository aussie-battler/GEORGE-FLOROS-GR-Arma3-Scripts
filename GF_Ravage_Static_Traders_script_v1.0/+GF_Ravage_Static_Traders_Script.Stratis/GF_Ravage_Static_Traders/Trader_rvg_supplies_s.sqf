...
diag_log "//________________ Trader_rvg_supplies_s.sqf _____________";
...
GF_Trader_supplies_Building_array = [
// = selectRandom [
"Land_TentA_F",
"Land_TentDome_F",
"Land_TentDome_F",
...
_Building = selectRandom GF_Trader_supplies_Building_array;
...
_Trader_supplies_group = createGroup civilian;
_Trader_supplies = _Trader_supplies_group createUnit ["C_Orestes", _Trader_Pos, [], 0, "FORM"];

//________________ Dress the trader with Ravage selections	________________
0 = [_Trader_supplies] call rvg_fnc_equip; 
_Trader_supplies setDir (random 360);

_Trader_supplies setVariable ["isTrader", "rvg_supplies_s", true];
_Trader_supplies setVariable ["GF_rvg_supplies_s", true];
...
[[_Trader_supplies,_ambient_Anim_array, "NONE"],BIS_fnc_ambientAnim ] remoteExec ["call"];
_Trader_supplies allowDamage false;
_Trader_supplies disableAI "MOVE";
...
{
		if (
		((_x getVariable ["GF_rvg_supplies_s",false]))		
		) then {
		_marker = createMarker ["GF_Trader_supplies" + str(_forEachIndex), getPos _x];
...
systemchat "S u p p l y   T r a d e r s   S p a w n e d";