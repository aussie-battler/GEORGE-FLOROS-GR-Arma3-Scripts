...
while {!_Found} do {
	...
	_nearestLocation = nearestLocation [_Pos, "NameCityCapital"];
...
systemchat format ["Position Found %1",_Found];
};

if (GF_Missions_Diag_Log_info) then {
diag_log format ["Position Found %1",_Found];
};
...
if (GF_Missions_Systemchat_info) then {
systemchat format ["There is NO NameCityCapital , spawning on random : %1",_locationPos]; 
};	

if (GF_Missions_Diag_Log_info) then {
diag_log format ["There is NO NameCityCapital , spawning on random : %1",_locationPos]; 
};
