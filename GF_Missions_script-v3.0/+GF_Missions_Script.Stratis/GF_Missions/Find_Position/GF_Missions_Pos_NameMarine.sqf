...
while {!_Found} do {
	...
	_Pos = [nil, ["ground"]] call BIS_fnc_randomPos;
	...
	_nearestLocation = nearestLocation [_Pos, "NameMarine"];
...
systemchat format ["Position NameMarine Found %1",_Found];
};

if (GF_Missions_Diag_Log_info) then {
diag_log format ["Position NameMarine Found %1",_Found];
};
...
if (GF_Missions_Systemchat_info) then {
systemchat format ["There is NO NameMarine , spawning on random : %1",_locationPos]; 
};	

if (GF_Missions_Diag_Log_info) then {
diag_log format ["There is NO NameMarine , spawning on random : %1",_locationPos]; 
};
...
waitUntil {_Found};
	...
	//________________	Get a random position , in water	________________
	_Pos = _locationPos getPos [random 100,random 360];
