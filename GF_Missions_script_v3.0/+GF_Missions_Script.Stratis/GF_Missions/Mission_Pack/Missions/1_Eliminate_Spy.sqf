...
diag_log "//________________	1_Eliminate_Spy.sqf	Initializing	_____________";
diag_log "________________	taskSetState	1_Eliminate_Spy_1	________________";
diag_log "________________	taskSetState	1_Eliminate_Spy_2	________________";
...
//________________ _Unit_Group	________________

_Unit_Group = createGroup civilian;
_Unit_Spy = _Unit_Group createunit [GF_Missions_Pool_Civilian select floor(random count GF_Missions_Pool_Civilian),_Pos_1,[],0,"None"];
units _Unit_Spy joinSilent _Unit_Group; 
_Unit_Spy setskill 1;

removeAllWeapons _Unit_Spy;
removeAllItems _Unit_Spy;
removeAllAssignedItems _Unit_Spy;
removeVest _Unit_Spy;
removeBackpack _Unit_Spy;
removeGoggles _Unit_Spy;

if (GF_Missions_Change_Equipment) then {	
_Unit_Spy spawn GF_SCL_GF_Missions_civilian;
};
...
_Unit_Spy addHeadgear _Headgear;
_Unit_Spy addGoggles _Goggles;

//	add possibility 
if (floor (random 10) < 4) then {_Unit_Spy addVest _Vests;};
if (floor (random 10) < 3) then {_Unit_Spy addBackpack _Backpacks;};

[_Unit_Group,_Pos_1,GF_Missions_BIS_fnc_taskPatrol_distance] call BIS_fnc_taskPatrol;




//________________ _Group_1	________________	

for "_x" from 0 to (random 10 + 15) do {

_Pos_Spawn =  [[[_Pos_1, 25 + random 100]],["water"]] call BIS_fnc_randomPos;
_Group_1 = createGroup civilian;

_Unit = _Group_1 createunit [GF_Missions_Pool_Civilian select floor(random count GF_Missions_Pool_Civilian),_Pos_Spawn,[],0,"None"];
[_Unit] JoinSilent _Group_1;
...
[_Group_1,_Pos_1,GF_Missions_BIS_fnc_taskPatrol_distance] call BIS_fnc_taskPatrol;
};
...
[GF_Missions_allPlayers,["1_Eliminate_Spy_1","GF_Missions_Pack"],["Eliminate the Spy.You need to get close , to reveal his identity !","Eliminate the Spy",""], _Mission_Pos,true,1,true,"kill",true] call BIS_fnc_taskCreate;
["1_Eliminate_Spy_1","ASSIGNED",true] spawn BIS_fnc_taskSetState;


diag_log "//________________	1_Eliminate_Spy.sqf	Initialized	_____________";

if (GF_Missions_Systemchat_info) then {
systemchat "1_Eliminate_Spy.sqf	Initialized";
};	
...
["1_Eliminate_Spy_1","FAILED",true] spawn BIS_fnc_taskSetState;
["1_Eliminate_Spy_2", "FAILED",true] spawn BIS_fnc_taskSetState;
...
//________________	Check Task	________________
	...
	waitUntil { { _x distance _Unit_Spy < 20 } count GF_Missions_allPlayers > 0 };
   
	if (GF_Missions_Systemchat_info) then {
	systemchat "distance _Unit_Spy < 20";
	};
	

	//________________	Change Behaviour	________________
	_Unit_Spy setBehaviour "STEALTH";	//	AWARE	STEALTH	COMBAT
	_Unit_Spy setCombatMode "YELLOW";		//	YELLOW	RED
	
	//________________	Change Side	________________	
	_Join_East = creategroup GF_Missions_Enemy_Side; 	
	units _Unit_Spy joinSilent _Join_East; 
	[_Join_East, _Pos_1] call BIS_fnc_taskAttack;
 
 	//________________	Arm	________________
	for "_i" from 1 to 2 do {_Unit_Spy addItemToUniform "30Rnd_45ACP_Mag_SMG_01";};
	_Unit_Spy addItemToUniform "FirstAidKit";
	_Unit_Spy addItemToUniform "SmokeShell";
	_Unit_Spy addItemToUniform "HandGrenade";
	_Unit_Spy addItemToUniform "MiniGrenade";

	comment "Add weapons";
	_Unit_Spy addWeapon "SMG_01_F";
	_Unit_Spy addPrimaryWeaponItem "muzzle_snds_acp";
	_Unit_Spy addPrimaryWeaponItem "optic_MRCO";
		

///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//________________	Check Task	________________
	
waitUntil {!alive _Unit_Spy;};
...
["1_Eliminate_Spy_1", "SUCCEEDED",true] spawn BIS_fnc_taskSetState;			
sleep 5;
...
[GF_Missions_allPlayers,["1_Eliminate_Spy_2","GF_Missions_Pack"],["The Spy has set a Trap! , Eliminate all threats! ","It is a Trap! Eliminate all threats!",""], _Pos_1,true,1,true,"kill",true] call BIS_fnc_taskCreate;
["1_Eliminate_Spy_2","ASSIGNED",true] spawn BIS_fnc_taskSetState;
...
["1_Eliminate_Spy_2", "SUCCEEDED",true] spawn BIS_fnc_taskSetState;
