...
diag_log "//________________	2_Eliminate_Spy_2.sqf	Initializing	_____________";
diag_log "________________	taskSetState	2_Eliminate_Spy_2_1	________________";
diag_log "________________	taskSetState	2_Eliminate_Spy_2_2	________________";


if (GF_Missions_Systemchat_info) then {
systemchat "2_Eliminate_Spy_2 Initializing";
};
...
_Unit_Group = createGroup GF_Missions_Enemy_Side;
_Unit = _Unit_Group createunit [GF_Missions_Pool_Civilian select floor(random count GF_Missions_Pool_Civilian),_Pos_1,[],0,"None"];
units _Unit joinSilent _Unit_Group; 
_Unit setskill 1;
...
for "_x" from 0 to (random 15 + 20) do {
...
//	J Zone belongs to Site > 0 = East, 1 = West, 2 = RACS, 3 = Guard (for more see readme page 7)
[0,	

//	K Unit configuration of the zone (DAC_Config_Units) > default units = 0 for East, 1 for West, 2 for RACS, 3 for Guards
5,	//	Custom editable Units in GF_Missions\Custom_Units_Array.sqf

//	L Behaviour configuration of the zone (DAC_Config_Behaviour) > default behaviour = 0 for East, 1 for West, 2 for RACS, 3 for Guard
0,	
...
[GF_Missions_allPlayers,["2_Eliminate_Spy_2_1","GF_Missions_Pack"],["Eliminate the Spy. Beware he is guarded","Eliminate the Spy",""], _Mission_Pos,true,1,true,"kill",true] call BIS_fnc_taskCreate;
["2_Eliminate_Spy_2_1","ASSIGNED",true] spawn BIS_fnc_taskSetState;


diag_log "//________________	2_Eliminate_Spy_2.sqf	Initialized	_____________";

if (GF_Missions_Systemchat_info) then {
systemchat "2_Eliminate_Spy_2.sqf	Initialized";
};	
...
["2_Eliminate_Spy_2_1","FAILED",true] spawn BIS_fnc_taskSetState;
...
["2_Eliminate_Spy_2_1", "SUCCEEDED",true] spawn BIS_fnc_taskSetState;			
sleep 5;
