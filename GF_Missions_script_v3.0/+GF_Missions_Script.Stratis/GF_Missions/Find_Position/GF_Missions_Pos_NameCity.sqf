...
while {!_Found} do {
	...
	_nearestLocation = nearestLocation [_Pos, "NameCity"];
...
if (GF_Missions_Systemchat_info) then {
systemchat format ["There is NO NameCity , spawning on random : %1",_locationPos]; 
};	

if (GF_Missions_Diag_Log_info) then {
diag_log format ["There is NO NameCity , spawning on random : %1",_locationPos]; 
};
