...
private ["_Pos" , "_Found" , "_Random_Building_Pos"];
...
while {!_Found} do {
	...
	_buildingPos_Found = false;
while {!_buildingPos_Found} do {		
	//________________	Get a random building position	________________

	_nearestObjects =  nearestObjects [_Pos, ["Building"], worldSize];
	_buildingPos = [];
	_nearestObjects apply {_buildingPos append (_x buildingPos -1)};
	
	if !(_buildingPos isEqualTo []) then {

	_Random_Building_Pos = selectRandom _buildingPos;
	_buildingPos_Found = true;
	
if (GF_Missions_Systemchat_info) then {
systemchat format ["Position buildingPos Found %1",_buildingPos_Found];
};	

if (GF_Missions_Diag_Log_info) then {
diag_log format ["Position buildingPos Found %1",_buildingPos_Found];
};

	} else{
	_buildingPos_Found = false;
	
	_Random_Building_Pos = _Pos;
	
if (GF_Missions_Systemchat_info) then {
systemchat format ["There is NO buildingPos , spawning on random : %1",_buildingPos_Found]; 
};	

if (GF_Missions_Diag_Log_info) then {
diag_log format ["There is NO buildingPos , spawning on random : %1",_buildingPos_Found]; 
};
...
if (GF_Missions_Systemchat_info) then {
systemchat format ["Position buildingPos Found %1",_Found];
};

if (GF_Missions_Diag_Log_info) then {
diag_log format ["Position buildingPos Found %1",_Found];
};
